...
const HEALTH_TIMEOUT_MS = 10000; // Increased for Railway
...
export async function GET() {
  try {
    ...
    const [whatsappStatus, discordStatus, waLogs, dcLogs] = await Promise.all([
      pingBotHealth(WHATSAPP_BOT_URL, '/api/whatsapp/health'),
      pingBotHealth(DISCORD_BOT_URL, '/api/discord/health'),
      fetchBotLogs(WHATSAPP_BOT_URL, '/api/whatsapp/messages?type=logs', 'whatsapp'),
      fetchBotLogs(DISCORD_BOT_URL, '/api/discord/messages?type=logs', 'discord'),
    ]);
