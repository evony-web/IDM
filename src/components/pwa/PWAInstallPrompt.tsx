...
/* ═══════════════════════════════════════════════════════════════════
   ...
   - Shows prompt on visits 2, 5, 9, 15 (max 4 times)
   - If user dismisses (click "nanti saja" or backdrop) → still show on next target visit
   - After visit 15 (not installed) → stop showing permanently
...
// Show prompt only on these specific visits
const TRIGGER_VISITS = [2, 5, 9, 15];
...
function getInstallDismissed(): boolean {
  ...
  // Permanently hidden only: installed OR max visits (15) exceeded
  // Dismiss does NOT permanently hide - will show again on next target visit
  return v === 'installed' || v === 'maxed_out';
}
...
export function PWAInstallPrompt() {
  ...
  useEffect(() => {
    ...
    // After max visit (15), permanently stop showing install prompt
    if (visits > TRIGGER_VISITS[TRIGGER_VISITS.length - 1]) {
      localStorage.setItem(STORAGE_KEY, 'maxed_out');
      return;
    }

    // Only show on specific target visit numbers (2, 5, 9, 15)
    const shouldShowPrompt = TRIGGER_VISITS.includes(visits);
  ...
  const handleInstall = useCallback(async () => {
    ...
    // If dismissed via install prompt, don't save - allow showing again on next target visit
  ...
  const handleDismiss = useCallback(() => {
    // Don't save dismiss - allow showing again on next target visit (5, 9, 15)
    setShowPrompt(false);
            ...
            >
              ...
              >
                <div
                  className="h-[1px]"
                  style={{
                    background: 'linear-gradient(90deg, transparent, rgba(56, 189, 248,0.4) 30%, rgba(56, 189, 248,0.6) 50%, rgba(56, 189, 248,0.4) 70%, transparent)',
                  }}
                />
                ...
                <div className="px-5 pb-5 pt-2">
                  ...
                  <div className="flex items-center gap-4 mb-5">
                    <div
                      ...
                      style={{
                        background: 'linear-gradient(135deg, rgba(56, 189, 248,0.15), rgba(212,201,31,0.06))',
