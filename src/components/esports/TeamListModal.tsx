...
export function TeamListModal({
  ...
  const accentBgSubtle = isMale ? 'bg-blue-500/12' : 'bg-[#0EA5E9]/12';
    ...
    <AnimatePresence>
      {isOpen && (
        ...
        >
          ...
          <motion.div
            className="relative w-full max-w-md lg:max-w-2xl max-h-[80vh] glass rounded-t-[32px] sm:rounded-[24px] overflow-hidden flex flex-col"
            initial={{ y: '100%' }}
            animate={{ y: 0 }}
            exit={{ y: '100%' }}
                  ...
                  {teams.map((team, i) => (
                    <motion.div
                      ...
                      whileHover={{
                        y: -2,
                        boxShadow: isMale
                          ? '0 8px 32px rgba(96,165,250,0.08), 0 2px 8px rgba(0,0,0,0.2)'
                          : '0 8px 32px rgba(244,114,182,0.08), 0 2px 8px rgba(0,0,0,0.2)',
                      }}
                    >
                      {/* Team header */}
                        ...
                        <div className={`relative flex-shrink-0`}>
                          <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-bold text-[13px] tracking-tight ${
                            isMale
                              ...
                              : 'bg-gradient-to-br from-[#38BDF8] to-[#0EA5E9] text-white/90'
                          }`}
                          style={{
                            boxShadow: isMale
                              ? '0 2px 8px rgba(96,165,250,0.3), inset 0 1px 0 rgba(255,255,255,0.3)'
                              : '0 2px 8px rgba(244,114,182,0.3), inset 0 1px 0 rgba(255,255,255,0.3)',
                          }}
                          >
                            {team.seed || i + 1}
                          </div>
