...
export function PlayerManagementScreen({
  ...
  // Achievement checking state
  const [checkingAchievementId, setCheckingAchievementId] = useState<string | null>(null);
  const [checkingAllAchievements, setCheckingAllAchievements] = useState(false);
  const [achievementResult, setAchievementResult] = useState<{ userId: string; userName: string; newCount: number; names: string } | null>(null);

  const isMale = division === 'male';
  const accentClass = isMale ? 'text-[--ios-gold]' : 'text-[--ios-pink]';
  const accentBgSubtle = isMale ? 'bg-[#73FF00]/12' : 'bg-[#0EA5E9]/12';
  ...
  const checkAchievement = async (userId: string, userName: string) => {
    setCheckingAchievementId(userId);
    try {
      ...
      if (data.success && data.newAchievements?.length > 0) {
        setAchievementResult({
          userId,
          userName,
          newCount: data.newAchievements.length,
          names: data.newAchievements.map((a: any) => `${a.icon} ${a.name}`).join(', '),
        });
        setTimeout(() => setAchievementResult(null), 5000);
      }
    } catch { /* silent */ }
    finally { setCheckingAchievementId(null); }
  };

  const checkAllAchievements = async () => {
    setCheckingAllAchievements(true);
    let totalNew = 0;
    ...
    for (const reg of approvedRegs) {
      try {
        const res = await fetch('/api/achievements/check', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ userId: reg.user.id }),
        });
        const data = await res.json();
        if (data.success) totalNew += data.newAchievements?.length || 0;
      } catch { /* silent */ }
    }
    setCheckingAllAchievements(false);
    if (totalNew > 0) {
      setAchievementResult({
        userId: 'all',
        userName: 'Semua Pemain',
        newCount: totalNew,
        names: `${totalNew} achievement baru terbuka!`,
      });
      setTimeout(() => setAchievementResult(null), 5000);
    }
  };
        ...
        >
          ...
          >
            ...
            <div className="flex-shrink-0 px-6 pt-14 pb-4">
              ...
              {currentMVP && (
                <motion.div
                  ...
                  className={`rounded-2xl p-4 mb-5 relative overflow-hidden ${
                    isMale
                      ? 'bg-gradient-to-r from-[#73FF00]/15 via-[#73FF00]/8 to-[#73FF00]/5 ring-1 ring-[#73FF00]/20'
                      : 'bg-gradient-to-r from-[#0EA5E9]/15 via-#0EA5E9/8 to-[#0EA5E9]/5 ring-1 ring-#0EA5E9/20'
                  }`}
                >
                  {/* Animated sparkle overlay */}
                  <div className={`absolute inset-0 opacity-20 ${
                    isMale
                      ? 'bg-gradient-to-r from-[#73FF00]/5 via-transparent to-[#73FF00]/5'
                      : 'bg-gradient-to-r from-[#38BDF8]/5 via-transparent to-[#38BDF8]/5'
                  }`} />
                  ...
                  <div className="relative flex items-center gap-3">
                    <div className="relative">
                      ...
                      >
                        <Crown className={`w-7 h-7 ${isMale ? 'text-[#73FF00]' : 'text-[#38BDF8]'}`} />
                      </motion.div>
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <p className={`text-[10px] font-bold uppercase tracking-[0.15em] ${isMale ? 'text-[#73FF00]/60' : 'text-[#38BDF8]/60'}`}>
                          MVP Saat Ini
                        </p>
                        <span className={`px-2.5 py-0.5 rounded-lg text-[9px] font-bold ${isMale ? 'bg-[#73FF00]/15 text-[#73FF00] border border-[#73FF00]/15' : 'bg-[#38BDF8]/15 text-[#38BDF8] border border-[#38BDF8]/15'}`}>
                          +25 pts
                        </span>
                      </div>
                      <p className="text-[15px] font-bold text-white/90 truncate mt-0.5">{currentMVP.user.name}</p>
                      {(currentMVP.user.mvpScore ?? 0) > 0 && (
                        <p className={`text-[11px] mt-1 font-medium ${isMale ? 'text-[#73FF00]/50' : 'text-[#38BDF8]/50'}`}>
                  ...
                  return (
                    <motion.button
                      ...
                      className={`rounded-2xl p-3 text-center transition-all duration-300 ${
                        activeFilter === tab.id
                          ? 'glass-subtle ring-1 ' + (isMale ? 'ring-[#73FF00]/25' : 'ring-[#38BDF8]/25')
              ...
              <div className="relative group">
                ...
                <input
                  ...
                  className={`w-full bg-white/5 border border-white/8 rounded-2xl pl-11 pr-10 py-3.5 text-white/90 text-[13px] lg:text-base font-medium placeholder-white/20 focus:outline-none transition-all duration-300 ${
                    isMale
                      ? 'focus:border-[#73FF00]/25 focus:bg-[#73FF00]/[0.02] focus:shadow-[0_0_24px_rgba(115,255,0,0.05)]'
                      : 'focus:border-[#38BDF8]/25 focus:bg-[#38BDF8]/[0.02] focus:shadow-[0_0_24px_rgba(244,114,182,0.05)]'
                  }`}
                />
                {search && (
                  <button
              ...
              {/* Bulk Actions — Check All Achievements */}
              {approvedCount > 0 && (
                <motion.button
                  onClick={checkAllAchievements}
                  disabled={checkingAllAchievements}
                  className="w-full mt-3 py-3 rounded-2xl text-[12px] font-bold flex items-center justify-center gap-2 transition-all duration-200 bg-amber-500/10 border border-amber-500/20 text-amber-400 hover:bg-amber-500/15 disabled:opacity-50 disabled:pointer-events-none"
                  whileTap={{ scale: 0.98 }}
                  initial={{ opacity: 0, y: -8 }}
                  animate={{ opacity: 1, y: 0 }}
                  transition={{ delay: 0.2 }}
                >
                  {checkingAllAchievements ? (
                    <Loader2 className="w-4 h-4 animate-spin" />
                  ) : (
                    <Award className="w-4 h-4" />
                  )}
                  {checkingAllAchievements ? `Mengecek ${approvedCount} pemain...` : 'Cek Achievement Semua Pemain'}
                </motion.button>
              )}

              {/* Achievement Result Toast */}
              <AnimatePresence>
                {achievementResult && (
                  <motion.div
                    ...
                    className="mt-3 p-3.5 rounded-2xl bg-gradient-to-r from-amber-500/15 via-amber-400/10 to-amber-500/5 border border-amber-500/20"
                  >
                    <p className="text-[11px] font-bold text-amber-300 mb-1">
                      🎉 {achievementResult.userName}
                    </p>
                    <p className="text-[10px] text-amber-400/70">
                      {achievementResult.names}
                    </p>
                  </motion.div>
                )}
              </AnimatePresence>
                    ...
                    return (
                      <motion.div
                        ...
                        className={`glass-subtle rounded-2xl p-4 lg:p-5 transition-all duration-300 ${
                          isMVP
                            ? (isMale ? 'ring-1 ring-[#73FF00]/25' : 'ring-1 ring-[#38BDF8]/25')
                            : ''
                        }`}
                      ...
                      >
                            ...
                            {isMVP && (
                              ...
                              >
                                <div className={`w-5 h-5 rounded-full flex items-center justify-center ${
                                  isMale ? 'bg-[#73FF00]' : 'bg-[#38BDF8]'
                                }`}
                                style={{ boxShadow: `0 2px 8px ${isMale ? 'rgba(96,165,250,0.4)' : 'rgba(244,114,182,0.4)'}` }}
                                >
                            ...
                            <div className="flex items-center gap-2 flex-wrap">
                              ...
                              {isMVP && (
                                <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-lg text-[9px] font-bold uppercase tracking-wider ${
                                  isMale ? 'bg-[#73FF00]/15 text-[#73FF00] border border-[#73FF00]/15' : 'bg-[#38BDF8]/15 text-[#38BDF8] border border-[#38BDF8]/15'
                                }`}>
                        ...
                        <AnimatePresence>
                          {isMvpInputting && (
                            ...
                            >
                              <div className={`rounded-2xl p-4 mb-3 ${isMale ? 'bg-[#73FF00]/[0.05] border border-[#73FF00]/15' : 'bg-[#0EA5E9]/[0.05] border border-#0EA5E9/15'}`}>
                                <div className="flex items-center justify-between mb-2.5">
                                  <p className="text-[10px] font-bold uppercase tracking-[0.15em] text-white/35 flex items-center gap-1.5">
                                    <Star className="w-3.5 h-3.5 text-[#73FF00]" />
                                    Input Skor MVP
                                  </p>
                                  <span className={`px-2.5 py-1 rounded-lg text-[9px] font-bold ${isMale ? 'bg-[#73FF00]/12 text-[#73FF00] border border-[#73FF00]/12' : 'bg-[#38BDF8]/12 text-[#38BDF8] border border-[#38BDF8]/12'}`}>
                                    Bonus: +25 pts (fixed)
                                  </span>
                                </div>
                                ...
                                <div className="relative">
                                  ...
                                  <input
                                    ...
                                    className={`w-full bg-white/5 border ${isMale ? 'border-[#73FF00]/15 focus:border-[#73FF00]/35' : 'border-#0EA5E9/15 focus:border-#0EA5E9/35'} rounded-xl pl-13 pr-4 py-3 text-white/90 text-[14px] font-bold placeholder-white/15 focus:outline-none transition-all duration-300 tabular-nums`}
                                ...
                                <div className="flex gap-2.5 mt-3.5 lg:gap-4">
                                  <motion.button
                                    ...
                                    className={`flex-1 py-3 rounded-xl text-[12px] font-bold flex items-center justify-center gap-1.5 transition-all duration-200 ${
                                      mvpScoreText
                                        ? (isMale
                                          ? 'bg-gradient-to-r from-[#73FF00] to-[#5FD400] text-black hover:from-[#8CFF33] hover:to-[#73FF00]'
                                          : 'bg-gradient-to-r from-[#38BDF8] to-[#0EA5E9] text-white/90 hover:from-[#7DD3FC] hover:to-[#38BDF8]')
                                        : 'bg-white/5 text-white/15 pointer-events-none'
                                    }`}
                                    whileTap={mvpScoreText ? { scale: 0.95 } : undefined}
                                    style={mvpScoreText ? {
                                      boxShadow: isMale ? '0 2px 12px rgba(115,255,0,0.25)' : '0 2px 12px rgba(56, 189, 248,0.25)',
                                    } : undefined}
                                  >
                        ...
                        <div className="flex flex-col gap-3">
                              ...
                              <div className="flex gap-1.5">
                                {['S', 'A', 'B'].map((tier) => (
                                  <motion.button
                                    key={tier}
                                    onClick={() => {
                                      // For approved players, use updateTier (just change tier)
                                      // For pending/rejected, use approve (change status + tier)
                                      if (isApproved) {
                                        onUpdateTier(reg.id, tier);
                                      } else {
                                        onApprove(reg.id, tier);
                                      }
                                    }}
                          ...
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            {isApproved && (
                              <motion.button
                                ...
                                className={`flex-1 min-w-0 py-2.5 rounded-xl text-[11px] font-bold flex items-center justify-center gap-1.5 transition-all duration-200 ${
                                  isMVP
                                    ? (isMale
                                      ? 'bg-[#73FF00]/15 text-[#73FF00] border border-[#73FF00]/25'
                                      : 'bg-[#38BDF8]/15 text-[#38BDF8] border border-[#38BDF8]/25')
                                    : (isMale
                                      ? 'bg-[#73FF00]/6 text-[#73FF00]/50 border border-[#73FF00]/12 hover:bg-[#73FF00]/12 hover:text-[#73FF00]'
                                      : 'bg-[#38BDF8]/6 text-[#38BDF8]/50 border border-[#38BDF8]/12 hover:bg-[#38BDF8]/12 hover:text-[#38BDF8]')
                                }`}
                                whileTap={{ scale: 0.95 }}
                              >
                            ...
                            {/* Check Achievement button — only for approved players */}
                            {isApproved && (
                              <motion.button
                                onClick={() => checkAchievement(reg.user.id, reg.user.name)}
                                disabled={checkingAchievementId === reg.user.id}
                                className={`py-2.5 px-3 rounded-xl text-[11px] font-bold flex items-center justify-center gap-1.5 transition-all duration-200 ${
                                  checkingAchievementId === reg.user.id
                                    ? 'bg-amber-500/20 text-amber-300 border border-amber-500/25'
                                    : 'bg-amber-500/8 text-amber-400/70 border border-amber-500/15 hover:bg-amber-500/15 hover:text-amber-400'
                                } disabled:opacity-50`}
                                whileTap={{ scale: 0.95 }}
                                title="Cek achievement pemain"
                              >
                                {checkingAchievementId === reg.user.id ? (
                            ...
                            {(isPending || isRejected) && (
                              <motion.button
                                ...
                                className={`flex-1 min-w-0 py-2.5 rounded-xl text-[11px] font-bold flex items-center justify-center gap-1.5 transition-all duration-200 ${
                                  isMale
                                    ? 'bg-[#73FF00]/10 text-[#73FF00] border border-[#73FF00]/15 hover:bg-[#73FF00]/18'
                                    : 'bg-[#38BDF8]/10 text-[#38BDF8] border border-[#38BDF8]/15 hover:bg-[#38BDF8]/18'
                                }`}
                                whileTap={{ scale: 0.95 }}
                              >
